...
def read_one(db: Session, sandwich_id):
    return db.query(models.Order).filter(models.Recipe.id == sandwich_id).first()


def update(db: Session, sandwich_id, recipe):
    # Query the database for the specific order to update
    db_recipe = db.query(models.Recipe).filter(models.Recipe.id == sandwich_id)
